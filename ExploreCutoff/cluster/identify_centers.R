...
hbwci <- identify_centers(TAZPloyNoNA, "totemp.den", den.cutoff.values[dc,"hbw"], dist=1.0, sum.col="tot.emp", sum.cutoff=tot.cutoff.values[dc,tc,"hbw"])
hbwci <- hbwci %>% dplyr::select(TAZ, center.id=cluster.id) %>% arrange(TAZ)
hbwci.name <- paste("hbwci", den.tot.cutoff.percentage, sep="")
assign(hbwci.name, hbwci)

# identify hbs tazs of centers 
hbsci <- identify_centers(TAZPloyNoNA, "st.hbs.den", den.cutoff.values[dc,"hbs"], dist=1.0, sum.col="st.hbs", sum.cutoff=tot.cutoff.values[dc,tc,"hbs"])
hbsci <- hbsci %>% dplyr::select(TAZ, center.id=cluster.id) %>% arrange(TAZ)
hbsci.name <- paste("hbsci", den.tot.cutoff.percentage, sep="")
assign(hbsci.name, hbsci)

# identify hbr tazs of centers 
hbrci <- identify_centers(TAZPloyNoNA, "st.hbr.den", den.cutoff.values[dc,"hbr"], dist=1.0, sum.col="st.hbr", sum.cutoff=tot.cutoff.values[dc,tc,"hbr"])
hbrci <- hbrci %>% dplyr::select(TAZ, center.id=cluster.id) %>% arrange(TAZ)
hbrci.name <- paste("hbrci", den.tot.cutoff.percentage, sep="")
assign(hbrci.name, hbrci)

# identify hbo tazs of centers 
hboci <- identify_centers(TAZPloyNoNA, "st.hbo.den", den.cutoff.values[dc,"hbo"], dist=1.0, sum.col="st.hbo", sum.cutoff=tot.cutoff.values[dc,tc,"hbo"])
hboci <- hboci %>% dplyr::select(TAZ, center.id=cluster.id) %>% arrange(TAZ)
hboci.name <- paste("hboci", den.tot.cutoff.percentage, sep="")
assign(hboci.name, hboci)
...
if (SAVE.INTERMEDIARIES) {
  intm.file <- file.path(INTERMEDIATE_DIR,  paste("centers", den.tot.cutoff.percentage, ".RData", sep=""))
  save(list=c(hbwci.name, hbsci.name, hbrci.name, hboci.name), file=intm.file)
}
