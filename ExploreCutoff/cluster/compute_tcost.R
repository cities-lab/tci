...
if (!in.memory(paste(Pr, "ci", den.tot.cutoff.percentage, sep="")))
  load(file.path(INTERMEDIATE_DIR, paste("centers", den.tot.cutoff.percentage, ".RData", sep="")))
...
#-------------------------------  
for (pr in Pr) {
  CentersObjName <- paste(pr, "ci", den.tot.cutoff.percentage, sep="")
