...
read.matdir <- function(bank, file0, mmat) {
	...
	mat.dir <- list()
	mat.dir <- c(mat.dir, list(ms=cbind(name=mat.name$ms, desc=mat.desc$ms, exists=cflag$ms[,1])))
	mat.dir <- c(mat.dir, list(mo=cbind(name=mat.name$mo, desc=mat.desc$mo, exists=cflag$mo[,1])))
	mat.dir <- c(mat.dir, list(md=cbind(name=mat.name$md, desc=mat.desc$md, exists=cflag$md[,1])))
	mat.dir <- c(mat.dir, list(mf=cbind(name=mat.name$mf, desc=mat.desc$mf, exists=cflag$mf[,1])))
...
read.mo <- function(numname, bank, file0, mcent, mat.dir) {
	...
	if (is.character(numname)) {
		...
		if (length(number)==1 && mat.dir$mo[number,3]==0) { stop("Matrix Not Found") }
                if(length(number)>1){
                  exist<-array(0,c(1,length(number)))
                  for (x in 1:length(number))
                    exist[x]<-as.integer(mat.dir$mo[number[x],3])
                  if(sum(exist)==0) { stop("Matrix Not Found") }
                  if(sum(exist)>1) {stop(paste("More than one matrix named mo",numname,sep=""))}
                  if(sum(exist)==1) {number<-number[which(exist==1)]}
                }
	} else { number <- numname }
...
read.md <- function(numname, bank, file0, mcent, mat.dir) {
	...
	if (is.character(numname)) {
	  numname <- gsub(" +$","",numname)
          ...
          if (length(number)==0) { stop("Matrix Not Found") }
          if (length(number)==1 && mat.dir$md[number,3]==0) { stop("Matrix Not Found") }
          if(length(number)>1){
            exist<-array(0,c(1,length(number)))
            for (x in 1:length(number))
              exist[x]<-as.integer(mat.dir$md[number[x],3])
            if(sum(exist)==0) { stop("Matrix Not Found") }
            if(sum(exist)>1) {stop(paste("More than one matrix named md",numname,sep=""))}
            if(sum(exist)==1) {number<-number[which(exist==1)]}
          }
...
read.mf <- function(numname, bank, file0, mcent, mat.dir) {
	...
	if (is.character(numname)) {
	  numname <- gsub(" +$","",numname)
          ...
          if (length(number)==0) { stop("Matrix Not Found") }
          if (length(number)==1 && mat.dir$mf[number,3]==0) { stop("Matrix Not Found") }
          if(length(number)>1){
            exist<-array(0,c(1,length(number)))
            for (x in 1:length(number))
              exist[x]<-as.integer(mat.dir$mf[number[x],3])
            if(sum(exist)==0) { stop("Matrix Not Found") }
            if(sum(exist)>1) {stop(paste("More than one matrix named mf",numname,sep=""))}
            if(sum(exist)==1) {number<-number[which(exist==1)]}
          }
        } else { number <- numname }
...
write.mf <- function(data, numname, bank, file0, mcent, mmat, mat.dir, newname=NULL, newdesc=NULL) {
	...
	## Write datestamp ##
        ...
        #seek(outfile, where=mmat*28+(number-1)*4, origin="current", rw="write")
		#Seek to matrix directory file 60 position
		seek(outfile, where=mat.offset*4, origin="start", rw="write")
		#Seek to timestamppart of matrix directory
		seek(outfile, where=(mmat*4*4), origin="current", rw="write")
		
		#Seek to name entry for mfs
		seek(outfile, where=mmat*3*4, origin="current", rw="write")
		#Seek to specific name entry for matrix
		seek(outfile, where=(number-1)*4, origin="current", rw="write")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
get_emme2_time <- function(timestamp){
