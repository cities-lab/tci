...
#Brian Gregor, gregor@or-analytics.com, 2/18/14
...
createFileOMX <- function( Filename, Numrows, Numcols, Level=1, Replace=FALSE ) {
  # Check inputs 
  if( class( Filename ) != "character" ){
    stop( "Filename must be a valid file name.")
  }
  if( Numrows != round(Numrows) ){
    stop( "Number of rows must be integer value.")
  }
  if( Numcols != round(Numcols) ){
    stop( "Number of columns must be integer value.")
  }
  if( file.exists(Filename) ) { 
    if( Replace == TRUE ) { 
      file.remove(Filename) 
    } else {
      stop( "File already exists. 'Replace' argument must be 'TRUE' if you want to replace the file.")
    }
  }
...
getRootAttrOMX <- function( OMXFileName ) {
	H5File <- H5Fopen( OMXFileName ) # Open the OMX file
	RootAttr <- list() # Create list to store results
  # Extract and save the SHAPE attribute
	H5Attr <- H5Aopen( H5File, "SHAPE" )
  RootAttr$SHAPE <- H5Aread( H5Attr )
  H5Aclose( H5Attr )
  # Extract and save the OMX_VERSION attribute
  H5Attr <- H5Aopen( H5File, "OMX_VERSION" )
  RootAttr$VERSION <- H5Aread( H5Attr )
  H5Aclose( H5Attr )
  # Close the OMX file and return the results
  H5Fclose( H5File )
  RootAttr
}
...
writeMatrixOMX <- function( OMXFileName, Matrix, MatrixSaveName, RowIndex=NULL, ColIndex=NULL, NaValue=-1, 
  ...
  if( !file.exists(OMXFileName) ){
    stop( paste( OMXFileName, "does not exist.") )
  }
  if( !is.matrix(Matrix) ){
    stop( "'Matrix' argument is not a matrix." )
  }
  if( !is.character(MatrixSaveName) ){
    stop( "'MatrixSaveName' argument must be a string." )
  }
  if( !is.character(Description) ){
    stop( "'Description' argument must be a string." )
  }
  # Get names of matrices in the file
...
writeLookupOMX <- function( OMXFileName, LookupVector, LookupSaveName, LookupDim=NULL, Replace=FALSE, Description="" ) {
  ...
  if( !is.character(LookupSaveName) ){
    stop( "'LookupSaveName' argument must be a string." )
  }
  if( !is.character(Description) ){
    stop( "'Description' argument must be a string." )
  }
...
listOMX <- function( OMXFileName ) {
  ...
  #Check whether there are any matrix contents and lookup contents
  HasMatrixContents <- nrow(MatrixContents) != 0
  HasLookupContents <- nrow(LookupContents) != 0
  #Read the matrix attribute information if any matrices exist
  if( HasMatrixContents ) {
  ...
  #Read the lookup attribute information if any lookups exist
	if( HasLookupContents ) {
	  H5File <- H5Fopen( OMXFileName )
    H5Group <- H5Gopen( H5File, "lookup" )
...
readMatrixOMX <- function( OMXFileName, MatrixName, RowIndex=NULL, ColIndex=NULL ) {
  ...
  if( !is.character(MatrixName) ){
    stop( "'MatrixName' argument must be a string." )
  }
  Matrices <- listOMX( OMXFileName )$Matrices$name
  if( !( MatrixName %in% Matrices ) ){
    stop( paste( MatrixName, "is not in", OMXFileName ) )
  }
  #Get the matrix dimensions specified in the file
	RootAttr <- getRootAttrOMX( OMXFileName )
	Shape <- RootAttr$SHAPE
  ...
  if( !is.null( RowIndex ) ) {
    if( class(RowIndex) != "integer" ) {
      stop( "'RowIndex' must be a vector of integers or 'NULL'" )
    }
    if( any( RowIndex <= 0 ) | ( max( RowIndex ) > Shape[1] ) ){
      stop( "One or more values of 'RowIndex' are outside the index range of the matrix." )
    }
  ...
  if( !is.null( ColIndex ) ) {
    if( class(ColIndex) != "integer" ){
      stop( "'ColIndex' must be a vector of integers or 'NULL'" )
    }
    if( !all( sapply( ColIndex, function(x) x == round(x) ) ) ) {
      stop( "'ColIndex' must be a vector of integers or 'NULL'" )
    }
    if( any( ColIndex <= 0 ) | ( max( ColIndex ) > Shape[2] ) ){
      stop( "One or more values of 'ColIndex' are outside the index range of the matrix." )
  ...
  Result <- t( h5read( OMXFileName, ItemName, index=Indices ) )
...
#Return: a lookup vector whose "DIM" attribute is set to the matrix dimension to which it applies.
readLookupOMX <- function( OMXFileName, LookupName ) {
  # Check inputs
  if( !file.exists(OMXFileName) ){
    stop( paste( OMXFileName, "does not exist.") )
  }
  if( !is.character(LookupName) ){
    stop( "'LookupName' argument must be a string." )
  }
  Lookups <- listOMX( OMXFileName )$Lookups$name
  if( !( LookupName %in% Lookups ) ){
    stop( paste( LookupName, "is not in", OMXFileName ) )
  }
  #Identify the item to be read
...
readSelectedOMX <- function( OMXFileName, MatrixName, RowSelection=NULL, ColSelection=NULL,
                             RowLookups=NULL, ColLookups=NULL ) {
	#Get the matrix dimensions specified in the file
	RootAttr <- getRootAttrOMX( OMXFileName )
	Shape <- RootAttr$SHAPE
  ...
  Matrix <- readMatrixOMX( OMXFileName, MatrixName, RowIndex=RowIndex, ColIndex=ColIndex )
  #Row lookup selections
  RowLookupValues <- list()
  for( LookupName in RowLookups ) {
    RowLookupValues[[LookupName]] <- readLookupOMX( OMXFileName, LookupName )[RowIndex]
  }
	#Column lookup selections
	ColLookupValues <- list()
	for( LookupName in ColLookups ) {
	  ColLookupValues[[LookupName]] <- readLookupOMX( OMXFileName, LookupName )[ColIndex]
	}
	#Return the matrix and row and column lookup selections
	list( Matrix=Matrix, RowLookupValues=data.frame(RowLookupValues), 
        ColLookupValues=data.frame(ColLookupValues) )                
}  
