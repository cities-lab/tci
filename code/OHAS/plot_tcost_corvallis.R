...
tcost.distr_tpurp.inc <- expand.grid(district.id=1:7, TripPurpose=c(Pr), inc.level=c(Ic), stringsAsFactors = F) %>% 
                         left_join(tcost.distr.tpurp.inc %>% ungroup() %>% mutate(inc.level=as.character(inc.level), TripPurpose=as.character(TripPurpose))) 

tcost.distr_tpurp.all <- expand.grid(district.id=1:7, TripPurpose=c(Pr), inc.level=c("All"), stringsAsFactors = F) %>%
                         left_join(mutate(ungroup(tcost.distr.tpurp), inc.level="All", TripPurpose=as.character(TripPurpose))) 

tcost.distr_all.inc <- expand.grid(district.id=1:7, TripPurpose=c("All"), inc.level=c(Ic), stringsAsFactors = F) %>%
                       left_join(mutate(ungroup(tcost.distr.inc), inc.level=as.character(inc.level), TripPurpose="All")) 

tcost.distr_all.all <- expand.grid(district.id=1:7, TripPurpose=c("All"), inc.level=c("All"), stringsAsFactors = F) %>%
...
tcost.distr_all <- rbind(tcost.distr_tpurp.inc, tcost.distr_tpurp.all, tcost.distr_all.inc, tcost.distr_all.all) %>%
                   right_join(expand.grid(district.id=1:7, TripPurpose=c('All', Pr), inc.level=c(Ic, 'All'), stringsAsFactors = F))
