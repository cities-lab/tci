...
# Part2 generates linked trips
  ...
  # Generate Linked trips 
    ...
    AGGACT <- c("Meals", "Work", "WorkRelted", "Shopping", "Shopping", "PersonalBus", "PersonalBus","PersonalBus",
...
#     
     ...
     linkedTrip <- identifyTripPurpose(act_out.2)
     ...
     #%>%
     #select(SAMPN, PERNO, ACTNO, HOME, LastHOME, AGGACT, LastAGGACT, OLOC, LastOLOC) 
...
#     linkedTrip$DistanceRoute <- with(linkedTrip, tdist[cbind(LASTRTZ, RTZ)] * 5280)
    ...
    for (i in c(1:nrow(linkedTrip))) {
      if (length(tdist[linkedTrip[i, "LastTAZ"], linkedTrip[i, "TAZ"]]) < 1) {
        
        linkedTrip[i, "DistanceRoute"] <- NA
      } else {
        
        linkedTrip[i, "DistanceRoute"] <- tdist[linkedTrip[i, "LastTAZ"], linkedTrip[i, "TAZ"]]*5280
