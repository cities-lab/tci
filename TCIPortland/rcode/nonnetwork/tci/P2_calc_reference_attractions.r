...
#This script identifies a reference zone for the travel cost index and related measures and computes 
#the reference market place attractions.

#The reference zone is identified as the TAZ that has the most attractions within it's market area. 
#The log sums of the JEMnR access utilities are used to identify market areas. 
#These measure the composite cost of travel between zones and are calculated in JEMnR 
#for each trip purpose and income group. The reference zone is identified as the zone 
#that has greatest market attractions, subject to limits of transit availability and 
#presence of households. Attraction scores are calculated by dividing the market attractions 
#for each zone by the maximum market attractions for all zones. This is done for each trip 
#purpose and income group. The results are summed for each zone to get a total score. 
#Since there are three income groups and four trip purposes, the maximum score is 12. 
#Reference market baskets are calculated for each combination of income and trip purpose. 
#Market baskets are calculated for each TAZ by calculating the percentage of trip attractions 
#to each TAZ and placing this vector of values in order of descending log sums. 
#Then a cumulative sum of the percentages is calculated and the zones whose values add to 
#50% are identified. This is the market area for the TAZ. The market basket for the TAZ 
#is calculated by summing the size terms from the destination choice model for TAZs in the market area. 
#The reference market basket is calculated as the mean of the market baskets for all zones.
...
#::
     ...
     calcMarketAccess <- function(Trips.Zi, SizeVar.Zi, LogSum.Zi, PctBreak=50){
          ...
          TripAttractions <- sum(SizeVar.Zi[MarketZones],na.rm = TRUE)
          ScoreAttractions <- sum(SizeVar.Zi[LogSum.Zi > 0.5],na.rm = TRUE)
...
#Iterate through each trip purpose and income group and call calcMarketAccess for each TAZ. 
#Create arrays of values by zone, income group and trip purpose. TripAttractions.ZiIcPr is an 
#array containing the values of total size terms that is used to calculate reference market 
#baskets. ScoreAttractions.ZiIcPr is an array containing the values of total size terms used 
#to identify the reference zone. The abbreviations after the period in the names of these 
#arrays indicates their dimensionality. Zi is a vector of the names of TAZs that are internal 
#to the model. Ic is a vector of the names of income groups. Pr is a vector of the names of 
#trip purposes.
...
#::
 ...
 ic<-Ic[1]
     ...
     for(pr in Pr){
          ...
          for(ic in Ic){
               DistFileName <- paste(codeLoc,"/tripdist/", pr, ic, "Dist.Rdata", sep="")
               load(DistFileName)
               rm(DistFileName)
               }
          ...
          LogsumFileName <- paste("intm_output/access/", "logSum", pr, ".Rdata", sep="")
          load(LogsumFileName)
          rm(LogsumFileName)
...
#::
     ...
     AttractionScoreQualified.Zi <- AttractionScore.Zi
...
#::
     ...
     save(TripAttractions.ZiIcPr, file="intm_output/reference/TripAttractions.ZiIcPr.RData")
     save(AttractionScore.Zi, file="intm_output/reference/AttractionScore.RData")
     save(AttractionScoreQualified.Zi, file="intm_output/reference/AttractionScoreQualified.Zi.RData")
     save(ReferenceZone, file="intm_output/reference/ReferenceZone.RData")
     save(ReferenceAttractions, file="intm_output/reference/ReferenceAttractions.RData")
