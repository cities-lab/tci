...
#============================         
     source("rcode/jemnr/PM_jemnrFunctions.R")
     source("rcode/jemnr/access/PM_accessUtilities.R")
     source("rcode/jemnr/access/PM_accessLogSum.R")
     attach(fun) 
...
#===========================================================
     ...
     source("rcode/nonnetwork/tci/calc_size_vars.R")
...
for(pr in Pr) {
    cMat <- readUtils(paste(codeLoc,"/inputs/", pr, "AccessUtils.csv", sep=""))
    assign(paste(pr, "AccessUtils", sep=""), cMat)
}
