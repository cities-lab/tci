...
#		    Twosubfolders:
#		       1. BaseYear
#			       -must contain emme2ban, PMInput.mac, TripLength.mac, hour.411, peak.411,and offpeak.411
#		       2. FutureYear
#			       -must contain emme2ban, PMInput.mac, TripLength.mac, hour.411, peak.411,and offpeak.411
#	2. Input
#		  -must contain LinkColNames.txt, timePeriods.csv
#	3. Output
...
#*******************************************************************************************************
#TODOLIST
#-Allow Base and Future SelectLinks to be different
#Replace lt with LinkTypes.Lk__


#--------------------------------------EXTRACT EMME2 DATA--------------------------------------------
#
#Set oldwd to shortcut's "Start in" location (eg:"C:\projects\RVCOG Performance Measures")

oldwd <- getwd()
SelectLink.Lk__ <- read.csv("Input\\SelectLinks.csv",header=TRUE,as.is=TRUE)
LinkTypes.Lk__ <- read.csv("Input\\LinkType.csv",header=FALSE,as.is=TRUE)

# Loop for Base and Future Year Scenarios
for(Year in c("BaseYear","FutureYear")){
...
#Run Emme2 Macro to create Emme2 Output file
      system(paste("emme2 -m PMInput.mac"),
...
#Run macro, find "BaseYear Average Trip Length"(or "FutureYear Average Trip Length")
      system(paste("emme2 -m TripLen.mac"),
        show.output.on.console=TRUE, invisible=TRUE)
...
#Run macro, find "BaseYear Average Trip Length"(or "FutureYear Average Trip Length")
       ...
       for(l in 1:length(SelectLink.Lk__[,1])){
          ...
          system(paste("emme2 -m SLTripLen.mac",fnode,tnode,sep=" "),
           show.output.on.console=TRUE, invisible=TRUE)
...
#Extract Average Trip Length value from emme2 file
          ...
          SLTripLen[l]<-as.numeric(SLTripLenFile[length(SLTripLenFile)])
          SelectLink.Lk__[,paste(Year,"SLTripLen",sep="")]<-SLTripLen
...
#Manipulate Links.Lk__ dataframe, add row and column names
      Links.Lk__<-Links.Lk__[,1:8]
      LinksCol.Lk__ <- read.table("Input\\LinkColNames.txt",as.is=TRUE)
      colnames(Links.Lk__)<- LinksCol.Lk__[1:8]
      rownames(Links.Lk__) <- paste(Links.Lk__$Inode, Links.Lk__$Jnode, sep="-")


      #SelectLink.Lk__<-SelectLink.Lk__[order(SelectLink.Lk__[,1]),]
      #SelectLink.Lk__$FC<-paste(LinkTypes.Lk__[Links.Lk__[rownames(Links.Lk__)%in%SelectLink.Lk__[,1],3],2], sep=" ")

#Read in Hourly demand factors
      dirunal.Per__ <- read.csv("Input\\timePeriods.csv",header=TRUE,as.is=TRUE)

#Run Loop to create 24 Hourly volumes using "Adt" and "Dirunal factors"
      for(i in 1:24){
            Links.Lk__[,paste("Hour",i,"Vol",sep="")]<-round(dirunal.Per__[i,4]*Links.Lk__$Adt,0)
      }
...
#Run Loop to create 24 Hourly Congestion Delay (in minutes) values (relative to FFS) for each link
      for(j in 1:24){
            Links.Lk__[,paste("CongDelay",j,sep="")] <-round(apply(Links.Lk__,1,function(x)calcHourlyDelay(x["Length"],
            x["Ffs"],x[paste("Hour",j,"Vol",sep="")], x["Lanes"], x["Cap"])),5)
      }
...
#Save a copy of current Links.Lk__ dataframe for use in RNCI Comparisons
      ...
      if (Year == "FutureYear"){
            FutureYearLinks.Lk__<- Links.Lk__}

#load RODBC and save results to MSAccess Database for reporting
      library(RODBC)
      channel <- odbcConnectAccess("RVCOGPM.mdb")
      sqlSave(channel,RVCOGSummPerfMeas,paste(Year,"RVCOG_SummaryPerformanceMeasures",sep=""),safer=FALSE)
      sqlSave(channel,SelectLink.Lk__,paste(Year,"RVCOG_SLTripLenMeasures",sep=""),safer=FALSE)
      sqlSave(channel,VC,paste(Year,"RVCOG_VCMeasures",sep=""),safer=FALSE)
      sqlSave(channel,AC,paste(Year,"RVCOG_ADTCMeasures",sep=""),safer=FALSE)
      sqlSave(channel,Links.Lk__,paste(Year,"RVCOG_LinkPerformanceMeasures",sep=""),safer=FALSE)
      odbcClose(channel)
      rm(channel)
...
#---------------------------------------------------------------
    for(lt in Lt[c(1:6)]){
          FileName <- paste("Output\\Base_Future Compare_RCNI", lt, ".emf", sep="")
          ...
          LinkAdtBLt <- FutureYearLinks.Lk__$Adt[FutureYearLinks.Lk__$Type == lt]
          BaseLanes <- BaseYearLinks.Lk__$Lanes[BaseYearLinks.Lk__$Type == lt]
          FutureLanes <- FutureYearLinks.Lk__$Lanes[FutureYearLinks.Lk__$Type == lt]
          plotRnci(LinkAdtALt, BaseLanes, main="Base Year")
          plotRnci(LinkAdtBLt, FutureLanes, main="Future Year")
...
#---------------------------------------------
     win.metafile("Output\\CompositeBase_Future Compare_RNCI.emf", width=10, height=7)
     OldPar <- par(mfrow=c(1,2), oma=c(1,1,4,1))
     ...
     plotRnciCap(BaseYearLinks.Lk__$Adt[IsMajor],BaseYearLinks.Lk__$Lanes[IsMajor],BaseYearLinks.Lk__$Cap[IsMajor],
               main="Base Year")
     plotRnciCap(FutureYearLinks.Lk__$Adt[IsMajor],FutureYearLinks.Lk__$Lanes[IsMajor],FutureYearLinks.Lk__$Cap[IsMajor],
               main="Future Year")
